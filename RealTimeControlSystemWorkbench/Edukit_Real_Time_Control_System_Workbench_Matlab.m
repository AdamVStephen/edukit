...
% in Edukit system for Load Disturbance measurement for Inverted
% Mode Operation
%
...
if v.Release == ("R2022a");
    nucleo = serialport(Serial_Port,230400);
    configureTerminator(nucleo, 'CR');
end
if v.Release ~= ("R2022a");
...
display_cycles = 1;
vert_axis_string = sprintf('Disturbance Sensitivity');
...
Sweep_Count_Threshold = 4;

% Set Enable_Record_All = 1 for default of 6 sweeps from start
Enable_Record_All = 0;
...
while exit_state == 0
    ...
    if Enable_Record_All == 1
        enable_data_archive = 1;
        fprintf(nucleo,'>');
        sweep_count_status = 1;
        Sweep_Count_Threshold = 10;
    end
    ...
    for k = 1:numpoints
        ...
        if enable_data_archive == 1 && sweep_count_status == 0
            ...
            try
                ...
                disp(sprintf('No entry of Sweep Count - assigning a value of 4'));
